...
// 创建并编译着色器
GLuint CreateShader(GLenum shaderType, const std::string code)//无符号四字节整型
{
    ...
    GLint codesLen[] = { (GLint)code.size() };//有符号四字节整型
    glShaderSource(shader, 1, codes, codesLen);//替换着色器中的代码，创建加载shader脚本源码
    glCompileShader(shader);//编译shader脚本源码

    GLint infoLength;
    glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &infoLength);//检测着色器编译是否成功
    if (infoLength != 0)
    {
        std::vector<char> info;
        info.reserve(infoLength + 1);//调整容量大小
        info.resize(infoLength);//调整vector大小

        GLsizei len;
        glGetShaderInfoLog(shader, infoLength, &len, &info[0]);//返回shader对象的日志信息
    ...
    GLint compileStatus;
    glGetShaderiv(shader, GL_COMPILE_STATUS, &compileStatus);//查询编译结果
...
GLuint CreateShaderProgram(const std::string vsFile, const std::string fsFile)
{
    ...
    if (vs == 0 || fs == 0)
    {
        if (vs != 0) { glDeleteShader(vs); }//删除一个shader对象
        if (fs != 0) { glDeleteShader(fs); }
        return 0;
    }

    GLuint prog = glCreateProgram();//创建着色器程序对象
    if (prog == 0)
    {
        glDeleteShader(vs);
        glDeleteShader(fs);
    ...
    glAttachShader(prog, vs);//附加一个shader对象到程序对象
    glAttachShader(prog, fs);
    glLinkProgram(prog);//链接一个程序对象

    GLint infoLength;
    glGetProgramiv(prog, GL_INFO_LOG_LENGTH, &infoLength);//返回程序对象参数
    if (infoLength != 0)
    {
        std::vector<char> info;
...
struct MMDGroundShadowShader//背景
{
...
struct Material
{
    ...
    GLuint    m_texture = 0;//纹理
    bool    m_textureHasAlpha = false;//无透明通道
...
struct Model
{
    ...
    void UpdateAnimation(const AppContext& appContext);//更新动画
...
{
    m_resourceDir = mmd::PathUtil::GetExecutablePath();//获得程序路径
    m_resourceDir = mmd::PathUtil::GetDirectoryName(m_resourceDir);//获得文件的路径名
    ...
    m_mmdShader = std::make_unique<MMDShader>();//智能指针
    ...
    //将本地数据传输到显卡纹理上
    glGenTextures(1, &m_dummyColorTex);//生成保存颜色数据的纹理
    glBindTexture(GL_TEXTURE_2D, m_dummyColorTex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);//分配存储空间
    ...
    m_copyTransparentWindowShaderTex = glGetUniformLocation(m_copyTransparentWindowShader, "u_Tex");//返回统一变量的位置序号

    glGenVertexArrays(1, &m_copyTransparentWindowVAO);//生成顶点数组对象名称
...
void AppContext::Clear()
{
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, 0);//指定绑定操作的帧缓冲区目标
    if (m_transparentFbo != 0) { glDeleteFramebuffers(1, &m_transparentFbo); }//删除帧缓存
    if (m_transparentMSAAFbo != 0) { glDeleteFramebuffers(1, &m_transparentMSAAFbo); }
    if (m_transparentFboColorTex != 0) { glDeleteTextures(1, &m_transparentFboColorTex); }//删除纹理
    if (m_transparentFboMSAAColorRB != 0) { glDeleteRenderbuffers(1, &m_transparentFboMSAAColorRB); }//删除渲染
...
void AppContext::SetupTransparentFBO()
{
    ...
    {
        glGenFramebuffers(1, &m_transparentFbo);//创建FBO
    ...
    if ((m_screenWidth != m_transparentFboWidth) || (m_screenHeight != m_transparentFboHeight))
    {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);//，为当前绑定的纹理对象设置过滤方式，将纹理像素映射成像素
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glBindTexture(GL_TEXTURE_2D, 0);

        glBindFramebuffer(GL_FRAMEBUFFER, m_transparentFbo);
        glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, m_transparentFboColorTex, 0);//将纹理对象绑定到帧缓冲区
        if (GL_FRAMEBUFFER_COMPLETE != glCheckFramebufferStatus(GL_FRAMEBUFFER))//返回帧缓冲区对象的帧缓冲区完整性状态，是否帧缓冲完成
        {
        ...
        glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_transparentFboMSAAColorRB);//将renderbuffer对象附加到framebuffer对象
...
Texture AppContext::GetTexture(const std::string & texturePath)
{
    ...
    if (it == m_textures.end())
    {
        ...
        if (comp != 4)
        {
            uint8_t* image = stbi_load_from_file(file.GetFilePointer(), &x, &y, &comp, STBI_rgb);//从文件中加载图片
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);//对齐像素字节
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, x, y, 0, GL_RGB, GL_UNSIGNED_BYTE, image);//指定二维纹理图像
            stbi_image_free(image);//释放
            hasAlpha = false;
        }
        else
        {
...
bool MMDShader::Setup(const AppContext& appContext)
{
    ...
    // 顶点属性
    m_inPos = glGetAttribLocation(m_prog, "in_Pos");//返回属性变量的位置
    ...
    m_uAmbinet = glGetUniformLocation(m_prog, "u_Ambient");//返回统一变量的位置//环境光
    m_uDiffuse = glGetUniformLocation(m_prog, "u_Diffuse");//漫反射光
    m_uSpecular = glGetUniformLocation(m_prog, "u_Specular");//镜面反射光
...
bool Model::Setup(AppContext& appContext)
{
    ...
    glGenBuffers(1, &m_posVBO);//生成缓冲区
    glBindBuffer(GL_ARRAY_BUFFER, m_posVBO);//绑定缓冲区对象
    glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3) * vtxCount, nullptr, GL_DYNAMIC_DRAW);//创建并初始化缓冲区对象的数据存储
    ...
    // 设置顶点数组对象
    glGenVertexArrays(1, &m_mmdVAO);//生成顶点数组对象
    glBindVertexArray(m_mmdVAO);//绑定顶点数组对象

    const auto& mmdShader = appContext.m_mmdShader;
    glBindBuffer(GL_ARRAY_BUFFER, m_posVBO);
    glVertexAttribPointer(mmdShader->m_inPos, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3), (const void*)0);//定义通用顶点属性数据的数组
    glEnableVertexAttribArray(mmdShader->m_inPos);//启用通用顶点
...
void Model::Clear()
{
    if (m_posVBO != 0) { glDeleteBuffers(1, &m_posVBO); }//删除缓冲区
    ...
    if (m_mmdVAO != 0) { glDeleteVertexArrays(1, &m_mmdVAO); }//删除顶点数组
...
void Model::Update(const AppContext& appContext)
{
    ...
    size_t vtxCount = m_mmdModel->GetVertexCount();
    glBindBuffer(GL_ARRAY_BUFFER, m_posVBO);
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3) * vtxCount, m_mmdModel->GetUpdatePositions());//更新缓冲区对象的数据存储的子集
    glBindBuffer(GL_ARRAY_BUFFER, m_norVBO);
...
void Model::Draw(const AppContext& appContext)
{
    ...
    wvit = glm::inverse(wvit);//求逆矩阵
    wvit = glm::transpose(wvit);//求矩阵的转置

    glActiveTexture(GL_TEXTURE0 + 3);//激活纹理单元
    glBindTexture(GL_TEXTURE_2D, appContext.m_dummyShadowDepthTex);//绑定纹理
    ...
    glEnable(GL_DEPTH_TEST);//启用功能，进行深度比较并更新深度缓冲区
    ...
    for (size_t i = 0; i < subMeshCount; i++)
    {
        ...
        glUseProgram(shader->m_prog);//使用程序对象作为当前渲染的一部分
        ...
        glUniform3fv(shader->m_uAmbinet, 1, &mmdMat.m_ambient[0]);//指定当前程序对象的统一变量的值
        ...
        if (mat.m_toonTexture != 0)
        {
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);//设置纹理参数GLint
        ...
        {
            glDisable(GL_CULL_FACE);//禁用功能，禁止多边形剔除
        }
        else
        {
            glEnable(GL_CULL_FACE);//启用多边形剔除
            glCullFace(GL_BACK);//指定是否可以剔除前面或后面的多边形
        }

        if (appContext.m_enableTransparentWindow)
        {
            glEnable(GL_BLEND);//启用将计算片段颜色值与颜色缓冲区中的值混合
            glBlendFuncSeparate(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ZERO, GL_ONE_MINUS_SRC_ALPHA);//分别指定RGB和alpha分量的像素计算
        }
        else
        {
            if (alphaBlend)
            {
                glEnable(GL_BLEND);
                glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);//指定像素算术
            }
            else
            {
                glDisable(GL_BLEND);
        ...
        glDrawElements(GL_TRIANGLES, subMesh.m_vertexCount, m_indexType, (GLvoid*)offset);//从数组数据中渲染图元
        ...
        glUseProgram(0);//使用程序对象作为当前渲染状态的一部分
    }
    ...
    // 阴影渲染
    glEnable(GL_POLYGON_OFFSET_FILL);//将偏移添加到由光栅化生产的多边形片段的深度值
    glPolygonOffset(-1, -1);//设置用于计算深度值的比例和单位
    ...
    if (appContext.m_enableTransparentWindow)
    {
        ...
        glStencilFuncSeparate(GL_FRONT_AND_BACK, GL_NOTEQUAL, 1, 1);//设置模板测试的前端和后端功能和参考值
        glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);//设置正面和背面模板测试操作
        glEnable(GL_STENCIL_TEST);//进行模板测试并更新模板缓冲区
    }
    else
    {
        ...
        {
            glEnable(GL_BLEND);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);//指定像素运算
...
bool SampleMain(std::vector<std::string>& args)
{
    ...
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3);//设置窗口提示参数
    ...
    glfwMakeContextCurrent(window);//绑定当前线程的content为window
    ...
    glfwSwapInterval(0);//设置swap间隔
    glEnable(GL_MULTISAMPLE);//使用多个片元采样来计算最终的像素颜色
    ...
    while (!glfwWindowShouldClose(window))//检查glfw是否被指示关闭
    {
        ...
        int width, height;
        glfwGetFramebufferSize(window, &width, &height);//改变窗口大小
        ...
        if (enableTransparentWindow)
        {
            ...
            glBindFramebuffer(GL_FRAMEBUFFER, 0);//绑定一个缓冲区对象
            ...
            glBlitFramebuffer(0, 0, width, height, 0, 0, width, height, GL_COLOR_BUFFER_BIT, GL_NEAREST);//将像素快从读取的帧缓冲区复制到绘制帧缓冲区
